...
const AddEditProduct = () => {
  ...
  return (
    <div className="min-h-screen flex bg-[#F8F9FC] font-sans text-slate-600">
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-x-hidden">
        ...
        <main className="p-4 md:p-8 max-w-4xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 sm:gap-8">
            {/* Form Side */}
            <div className="lg:col-span-3 bg-white rounded-2xl sm:rounded-[2.5rem] p-6 sm:p-8 shadow-xl shadow-slate-200/50 border border-white">
              ...
              <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
                <div className="space-y-4">
                  <div>
                    ...
                    <input type="text" required value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})}
                      className="w-full p-3 sm:p-3 bg-slate-50 border-2 border-transparent focus:border-blue-500 focus:bg-white rounded-xl sm:rounded-2xl outline-none transition-all font-bold text-sm sm:text-base text-slate-700"
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-3 sm:gap-4">
                    <div>
                      ...
                      <input type="number" required value={formData.price} onChange={(e) => setFormData({...formData, price: e.target.value})}
                        className="w-full p-3 sm:p-3 bg-slate-50 border-2 border-transparent focus:border-blue-500 focus:bg-white rounded-xl sm:rounded-2xl outline-none transition-all font-bold text-sm sm:text-base"
                      />
                    </div>
                    <div>
                      <label className="text-[10px] sm:text-xs font-black uppercase text-slate-400 ml-1">Category</label>
                      <select value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})}
                        className="w-full p-3 sm:p-3 bg-slate-50 border-2 border-transparent focus:border-blue-500 focus:bg-white rounded-xl sm:rounded-2xl outline-none transition-all font-bold text-sm sm:text-base appearance-none cursor-pointer"
                      >
                  ...
                  <div>
                    ...
                    <textarea rows="4" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})}
                      className="w-full p-3 sm:p-3 bg-slate-50 border-2 border-transparent focus:border-blue-500 focus:bg-white rounded-xl sm:rounded-2xl outline-none transition-all font-medium text-sm sm:text-base"
                    ></textarea>
                  </div>
                </div>
                ...
                <button type="submit" disabled={isSubmitting}
                  className="w-full py-3 sm:py-4 bg-[#1d3557] text-white rounded-xl sm:rounded-2xl font-black flex items-center justify-center gap-3 shadow-xl shadow-blue-900/20 hover:bg-[#2a4a75] hover:-translate-y-1 active:scale-95 transition-all disabled:opacity-50 text-sm sm:text-base cursor-pointer disabled:cursor-not-allowed"
                >
      ...
      {/* --- SUCCESS MODAL - Resized & Compact --- */}
      {showSuccessModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-[#1d3557]/40 backdrop-blur-md animate-in fade-in duration-300"></div>
          <div className="relative bg-white rounded-3xl w-full max-w-sm p-6 text-center shadow-2xl animate-in zoom-in-90 slide-in-from-bottom-10 duration-500">
            <div className="relative mx-auto w-12 h-12 sm:w-16 sm:h-16 mb-4">
              <div className="absolute inset-0 bg-green-100 rounded-full animate-ping opacity-25"></div>
              <div className="relative bg-green-500 text-white p-3 rounded-full inline-block shadow-lg shadow-green-200">
                <CheckCircle size={24} className="sm:size-8" strokeWidth={3} />
              </div>
            </div>
            
            <h3 className="text-xl sm:text-2xl font-black text-slate-800 mb-2 tracking-tight">Amazing!</h3>
            ...
            <button 
              ...
              className="group w-full py-3 bg-[#1d3557] text-white rounded-xl font-black uppercase tracking-widest text-xs flex items-center justify-center gap-2 shadow-lg shadow-blue-900/20 hover:bg-blue-600 transition-all cursor-pointer"
            >
              Back to Menu <ChevronRight size={14} className="group-hover:translate-x-1 transition-transform" />
            </button>
          </div>
        </div>
      )}

      {/* --- LOGOUT MODAL - Resized & Compact --- */}
      {showLogoutModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300"></div>
          <div className="relative bg-white rounded-2xl w-full max-w-xs p-6 text-center shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="p-3 bg-red-50 rounded-2xl inline-block mb-4 text-red-500">
              <LogOut size={24} />
            </div>
            <h3 className="text-lg font-black text-slate-800 mb-2">Logout?</h3>
            <p className="text-xs text-slate-400 font-medium mb-6">Ready to leave the kitchen?</p>
            <div className="flex flex-col sm:flex-row gap-3">
              <button onClick={cancelLogout} className="w-full sm:flex-1 py-3 font-black text-slate-400 uppercase text-[10px] border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-50">Wait, no</button>
              <button onClick={confirmLogout} className="w-full sm:flex-1 py-3 bg-red-500 text-white rounded-xl font-black uppercase text-[10px] shadow-lg shadow-red-200 hover:bg-red-600 transition-all cursor-pointer">Yes, Logout</button>
            </div>
          </div>
        </div>
      )}
